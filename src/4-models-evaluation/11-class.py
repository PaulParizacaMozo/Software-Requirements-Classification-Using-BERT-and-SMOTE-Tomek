...
# Filtrar solo los requisitos no funcionales (NF)
# Lista de clases no funcionales
nf_classes = ['A', 'L', 'LF', 'MN', 'O', 'PE', 'SC', 'SE', 'US', 'FT', 'PO']

# Los requisitos funcionales se eliminan de la clasificación de 11 clases
df['_class_'] = df['_class_'].apply(lambda x: x if x in nf_classes else None)

# Eliminar los requisitos funcionales de la base de datos
...
output_file = os.path.join(output_dir, 'results_classification_NF_11_classes.csv')
