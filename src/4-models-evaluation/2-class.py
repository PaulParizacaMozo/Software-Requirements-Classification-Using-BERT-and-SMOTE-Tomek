...
# Reemplazar las clases no funcionales (NF) con 'NF'
# Lista de clases no funcionales
nf_classes = ['A', 'L', 'LF', 'MN', 'O', 'PE', 'SC', 'SE', 'US', 'FT', 'PO']

# Cambiar todas las clases no funcionales a 'NF', y dejar 'F' tal cual
df['_class_'] = df['_class_'].apply(lambda x: 'F' if x == 'F' else 'NF')
...
output_file = os.path.join(output_dir, 'results_classification_F_NF.csv')
